...
class_table<- metadata %>% 
            ...
            filter(Category== "RecordLevel", InDES=="x") %>% 
            select(-InDES)
kable(class_table, caption = "Record Level Class")
```
...
class_table<- metadata %>% 
            select(Category, InDES, Term,Description,Examples, DataType ) %>% 
            filter(Category== class, InDES=="x") %>% 
            select(-InDES)
kable(class_table, caption = class)
...
vocabulary<- metadata %>% 
                  select(Category, MeasurementID, SubsetOfMetrics,
...
```{r echo=FALSE}
crosswalk<- metadata %>% 
        select(c("Category", "MeasurementID", "VocabularyCatagory", "SubsetOfMetrics", "InDES", 
